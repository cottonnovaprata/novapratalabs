...
export default function TicketsPage() {
  ...
  const fetchData = React.useCallback(async () => {
    setLoading(true)
    try {
      const [tRes, aRes] = await Promise.all([fetch("/api/tickets"), fetch("/api/assets")])
  ...
  async function handleCreateOrUpdate(data: any) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        success(editingTicket ? "Chamado atualizado" : "Chamado aberto"); fetchData()
      } else {
        const result = await res.json().catch(() => null)
        toastError(result?.error || "Erro ao salvar chamado")
      }
    } catch { toastError("Erro ao salvar chamado") }
  }
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      if (res.ok) { success("Chamado excluído"); fetchData() }
      else { const r = await res.json().catch(()=>null); toastError(r?.error || "Erro ao excluir") }
    } catch { toastError("Erro ao excluir chamado") }
  }
  ...
  const openNewTicket = () => { setEditingTicket(null); setIsModalOpen(true) }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button className="bg-primary shadow-lg shadow-primary/20" onClick={openNewTicket}>
      ...
      <div className="grid gap-6 md:grid-cols-4">
        <Card className="bg-amber-500/5 border-amber-500/20">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium flex items-center gap-2"><Clock className="h-4 w-4 text-amber-500" /> Chamados em Aberto</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold">{String(openCount).padStart(2,'0')}</div></CardContent>
        </Card>
        <Card className="bg-red-500/5 border-red-500/20">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-red-500" /> Críticos em Aberto</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold">{String(criticalOpenCount).padStart(2,'0')}</div></CardContent>
        </Card>
        <Card className="bg-emerald-500/5 border-emerald-500/20">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-emerald-500" /> Resolvidos</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold">{String(closedCount).padStart(2,'0')}</div></CardContent>
        </Card>
        <Card className="bg-rose-500/5 border-rose-500/20">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-rose-500" /> SLA Estourado</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold">{String(slaBreachedCount).padStart(2,'0')}</div></CardContent>
        </Card>
      </div>

      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div><CardTitle>Lista de Chamados</CardTitle><CardDescription>Todos os chamados registrados no sistema.</CardDescription></div>
          ...
          <div className="divide-y relative min-h-[300px]">
            {loading ? (
              ...
              <EmptyState
                ...
                description="Quando um chamado for aberto, ele aparecerá aqui com status, prioridade e SLA."
                action={<Button onClick={openNewTicket}><Plus className="mr-2 h-4 w-4" />Abrir primeiro chamado</Button>}
              />
            ) : (
              tickets.map((t) => (
                <div key={t.id} className="p-4 hover:bg-muted/50 transition-colors flex items-center justify-between group">
                  <div className="flex items-center gap-4">
                    <div className={cn("p-2 rounded-full", t.status === "CONCLUIDO" ? "bg-emerald-500/10 text-emerald-500" : "bg-amber-500/10 text-amber-500")}>
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge className={cn("text-[10px] font-normal py-0 h-4 border", PRIORITY_STYLE[t.priority])}>{t.priority}</Badge>
                        <Badge className={cn("text-[10px] font-normal py-0 h-4 border", STATUS_STYLE[t.status] ?? "bg-zinc-500/10 text-zinc-400 border-zinc-500/20")}>{t.status}</Badge>
                        {t.recurring && <Badge variant="outline" className="text-[10px] font-normal py-0 h-4 text-red-400 border-red-500/30">Recorrente</Badge>}
                        {(() => { const sla = getSlaStatus(t); return (
                          <Badge className={cn("text-[10px] font-normal py-0 h-4 border",
                            sla.isResolved ? (sla.isBreached ? "bg-zinc-500/10 text-zinc-400 border-zinc-500/20" : "bg-emerald-500/10 text-emerald-500 border-emerald-500/20")
                            : (sla.isBreached ? "bg-red-500/10 text-red-500 border-red-500/20" : "bg-blue-500/10 text-blue-500 border-blue-500/20"))}>
                            SLA: {sla.label}
                          </Badge>
                        )})()}
                      </div>
                  ...
                  <div className="flex items-center gap-2">
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-primary" aria-label="Editar chamado" onClick={() => { setEditingTicket(t); setIsModalOpen(true) }}><Pencil className="h-4 w-4" /></Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" aria-label="Excluir chamado" onClick={() => handleDelete(t.id)}><Trash2 className="h-4 w-4" /></Button>
                  </div>
                </div>
              ))
            )}
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingTicket ? "Editar Chamado" : "Novo Chamado"}>
        <TicketForm initialData={editingTicket} assets={assets} onCancel={() => setIsModalOpen(false)} onSubmit={handleCreateOrUpdate} />
      </Modal>
    </div>
  )
}
